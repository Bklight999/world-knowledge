#!/usr/bin/env bash
#
# data_pipeline_train_test_only.sh — keep only Test + Judge (Phase 2 + Phase 3),
# skip Notebook generation.
#
# Prerequisite: Notebooks have already been generated (Phase 1 artifacts exist).
# This script runs CK-pro Test + Judge + Analysis directly.
#
# Web-server port pool: 32 servers (3001-3032), each holding up to MAX_PER_SERVER
# concurrent tasks. Tasks fetch a port from a FIFO (blocking), and return it when
# done (first-in / first-out scheduling).
#
# Usage:
#   bash data_pipeline_train_test_only.sh [batch_size]
#
#   Edit the "urls" array or the "domain" variable below to choose what to test.
#
# Environment variables:
#   MAX_PER_SERVER - max concurrent tasks per web server (default: 1)
#   TOKEN_LIMIT    - guidebook token limit (default: 32000)
#   MODE           - test mode (default: urls)
...
#
# Examples:
#   bash data_pipeline_train_test_only.sh
#   bash data_pipeline_train_test_only.sh 3
#   MAX_PER_SERVER=2 bash data_pipeline_train_test_only.sh 5
...
LOG_FILE="./pipeline_log/run_train_test_only.log"

# Cleanup on exit / interrupt
trap 'echo "Stopping all tasks..."; kill $(jobs -p) 2>/dev/null; exec 4>&- 2>/dev/null; exit' SIGINT SIGTERM EXIT

# Make sure the log directory exists
mkdir -p "$(dirname "$LOG_FILE")"
: > "$LOG_FILE"

echo "===== Start run [train mode - TEST+JUDGE ONLY, MODE=${MODE}] at $(date) =====" | tee -a "$LOG_FILE"
...
case "$MODE" in
    ...
    domain)
        ...
        for _cf in "${_cluster_files[@]}"; do
            # Take the first [Prefix] line's URL as the cluster's base URL
            _base=$(grep -m1 -E '^\[Prefix\][[:space:]]+https?://' "$_cf" \
                    | awk '{print $2}')
            if [[ -z "$_base" ]]; then
            ...
            # Force a trailing slash to stay consistent with the "urls" array style
            [[ "$_base" != */ ]] && _base="${_base}/"
...
###############################################
# 0-b. Filter: drop URLs that have no .md guidebook file.
#   .md path: ./questions/{url_safe_name}_final_guidebook_{TOKEN_LIMIT}_{id}.md
#   Keep a URL as long as at least one NOTE_ID has an .md file.
###############################################
...
for url in "${raw_urls[@]}"; do
    ...
    _has_md=false
    for _check_id in "${NOTE_IDS[@]}"; do
        if [[ "$_check_id" -eq 6 ]]; then
            _has_md=true
            echo "  [FOUND] ${_name_test} ID=${_check_id}: no-notebook mode (always run)" | tee -a "$LOG_FILE"
            continue
        fi
        _md_file="${QUESTIONS_DIR}/${_name_test}_final_guidebook_${TOKEN_LIMIT}_${_check_id}.md"
        if [[ -f "$_md_file" ]]; then
            _has_md=true
            echo "  [FOUND] ${_name_test} ID=${_check_id}: $_md_file" | tee -a "$LOG_FILE"
        else
            echo "  [MISS]  ${_name_test} ID=${_check_id}: $_md_file not found" | tee -a "$LOG_FILE"
        fi
    done

    if [[ "$_has_md" == "false" ]]; then
        echo "  [SKIP] No .md guidebook found for any ID: ${_name_test}" | tee -a "$LOG_FILE"
        continue
    fi
...
if [[ ${TOTAL_URLS} -eq 0 ]]; then
    echo "[ERROR] No valid URLs with .md guidebook found. Exiting." | tee -a "$LOG_FILE"
    exit 1
fi

# --- vLLM ports --- use 8080-8083 and round-robin across test tasks
...
for (( batch=0; batch<TOTAL_BATCHES; batch++ )); do
    ...
    ###############################################
    # Phase 1: Problem Generation with Notebook (CPU only, no web server)
    #   Runs only for (URL, ID) pairs whose .md guidebook exists.
    #   .md path: ./questions/{url_safe_name}_final_guidebook_{TOKEN_LIMIT}_{id}.md
    ###############################################
    echo "[Batch ${BATCH_NUM}] [Phase 1] Problem Generation with Notebook at $(date)" | tee -a "$LOG_FILE"
    ...
    for url in "${batch_urls[@]}"; do
        ...
        for id in "${NOTE_IDS[@]}"; do
            if [[ "$id" -ne 6 ]]; then
                _md_file="${QUESTIONS_DIR}/${url_name_test}_final_guidebook_${TOKEN_LIMIT}_${id}.md"
                if [[ ! -f "$_md_file" ]]; then
                    echo "  [Phase 1] SKIP ${url_name_test} ID=${id} — no .md file" | tee -a "$LOG_FILE"
                    continue
                fi
            fi

            echo "  [Phase 1] RUN  ${url_name_test} ID=${id}" | tee -a "$LOG_FILE"
            (
                python3 problem_generation_with_notebook.py \
                    --notebook_id "${id}" \
                    --url "${url}" \
                    --token_limit "${TOKEN_LIMIT}" >> "$LOG_FILE" 2>&1
            ) &
            PHASE1_PIDS+=($!)
        done
    done
    ...
    echo "[Batch ${BATCH_NUM}] [Phase 1] Problem Generation finished at $(date)" | tee -a "$LOG_FILE"

    ###############################################
    # Phase 2: CK-pro Test + Judge
    #   Every (URL, ID) pair runs in parallel, scheduled via the FIFO port pool.
    #   Pairs without a matching .md guidebook are skipped.
    ###############################################
    echo "[Batch ${BATCH_NUM}] [Phase 2] CK-pro Test + Judge (${#ALIVE_PORTS[@]} servers x ${MAX_PER_SERVER} slots) at $(date)" | tee -a "$LOG_FILE"

    # Count the tasks that will actually run (i.e. have an .md file)
    BATCH_TOTAL_TASKS=0
    for url in "${batch_urls[@]}"; do
        _nt=$(url_to_safe_name "$url")
        for id in "${NOTE_IDS[@]}"; do
            if [[ "$id" -eq 6 ]] || [[ -f "${QUESTIONS_DIR}/${_nt}_final_guidebook_${TOKEN_LIMIT}_${id}.md" ]]; then
                BATCH_TOTAL_TASKS=$((BATCH_TOTAL_TASKS + 1))
            fi
        done
    done
    ...
    echo "[Batch ${BATCH_NUM}] [Phase 2] ${BATCH_TOTAL_TASKS} tasks to run (filtered by .md)" | tee -a "$LOG_FILE"
    ...
    for url in "${batch_urls[@]}"; do
        ...
        for id in "${NOTE_IDS[@]}"; do
            # Skip the (URL, ID) pair when no .md file exists.
            # ID=6 is the "no-notebook" test mode and does not require an .md.
            if [[ "$id" -ne 6 ]]; then
                _md_file="${QUESTIONS_DIR}/${url_name_test}_final_guidebook_${TOKEN_LIMIT}_${id}.md"
                if [[ ! -f "$_md_file" ]]; then
                    echo "  [Phase 2] SKIP ${url_name_test} ID=${id} - no .md file" | tee -a "$LOG_FILE"
                    continue
                fi
            fi
            ...
            (
                ...
                echo "[ID $id] Starting Phase 2 (test+judge)..." | tee -a "$task_log"
                ...
                # problem_generation_with_notebook.py may have failed to produce this file.
                if [[ ! -f "$INPUT_FILE" ]]; then
                    echo "[ID $id] ERROR: Input file not found: $INPUT_FILE. Skipping." | tee -a "$task_log"
                    _done=$(flock "$BATCH_DONE_FILE" bash -c "n=\$(cat '$BATCH_DONE_FILE'); echo \$((n+1)) > '$BATCH_DONE_FILE'; echo \$((n+1))")
                    echo "[Batch ${BATCH_NUM}] [Progress] ${_done}/${BATCH_TOTAL_TASKS} - [ID $id @ $url_name_raw] SKIPPED (no input)" | tee -a "$LOG_FILE"
                    exit 0
                fi
    ...
    # Wait for every Phase 2 task in the current batch to finish
    wait "${PHASE2_PIDS[@]}"
    rm -f "$BATCH_DONE_FILE"
    echo "[Batch ${BATCH_NUM}] All Phase 2 (Test + Judge) tasks finished at $(date)" | tee -a "$LOG_FILE"
...
echo "===== All ${TOTAL_BATCHES} Batches Finished [train mode - TEST+JUDGE ONLY, ${TOTAL_URLS} URLs] at $(date) =====" | tee -a "$LOG_FILE"
