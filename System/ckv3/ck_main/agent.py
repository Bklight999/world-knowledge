...
class CKAgent(MultiStepAgent):
    def __init__(self, **kwargs):
        ...
        feed_kwargs = dict(
            ...
            active_functions=["web_agent", "file_agent", "stop", "ask_llm"],  # enable the useful modules
            sub_agent_names=["web_agent", "file_agent"],  # note: another tricky point, use name rather than the objects themselves
            tools=[StopTool(agent=self), self.tool_ask_llm],  # add related tools
            max_steps=200,  # still give it more steps
            max_time_limit=43200,  # 1day
            exec_timeout_with_call=1000,  # if calling sub-agent
            exec_timeout_wo_call=300,  # if not calling sub-agent
        )
