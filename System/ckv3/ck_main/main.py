...
def get_args():
    ...
    parser.add_argument("--evaluation-metric", choices=["em", "llm_score"], default="em") # em: GAIA scorer based on exact-match. llm_score: using llm-based similarity match
...
def main():
    ...
    with my_open_with(args.output, 'w') as fout:
        ...
        with open(args.input) as fd:
            for line in fd:
                task_num += 1
        progress_file = os.path.join(os.path.dirname(args.output), f"progress_{args.output.split('/')[-1].split('_')[1].split('.')[0]}.txt")
        task_start_time = time.time()
        for inst in yield_inputs(args.input):
            with open(progress_file, 'a') as f:
                f.write(f"{inst['id']} {task_num} (Current Time: {time.ctime()}, Elapsed Time: {time.time() - task_start_time:.2f}s)\n")
            _input_file = None
