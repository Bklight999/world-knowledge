...
class MultiStepAgent(KwargsInitializable):
    def __init__(self, **kwargs):
        ...
        self.max_steps = 350  # maximum steps
        self.max_time_limit = 0  # early stop if exceeding this time (in seconds)
        self.recent_steps = 3  # feed recent steps
    ...
    def step(self, session, state):
        ...
        if has_plan_template:  # planning to update state
            plan_messages = self.templates["plan"].format(**_input_kwargs) #把任务信息，上一step的信息，当前step的信息，包装成prompt
    ...
    # 把任务描述，状态，最近的步骤，当前的步骤，subagent，工具等信息准备好
