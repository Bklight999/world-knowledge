...
class MessageTruncator:
    def __init__(self, model_name="/apdcephfs_fsgm/share_303846923/user/qifanzhang/notebook-qwen3-30b/full/sft_self_evolve_round2"):
...
class OpenaiHelper:
    ...
    def get_openai_client(model_name="", api_endpoint="", api_key=""):
        ...
        if model_name not in OpenaiHelper._openai_clients:  # lazy init
            ...
            rprint(f"client_api_key: {client.api_key}")
            rprint(f"client_base_url: {client.base_url}")
...
class GeminiHelper:
    @staticmethod
    def call_chat(messages, stat=None, **gemini_kwargs):
        pass
...
class LLM(KwargsInitializable):
    def __init__(self, **kwargs):
        ...
        self.max_retry_times = 10  # <0 means always trying
        self.seed = 1377  # zero means no seed!
        # request
        self.request_timeout = 3000  # timeout time
        self.max_token_num = 200000
        self.call_kwargs = {"temperature": 0.3, "top_p": 0.95, "max_tokens": 8192, "repetition_penalty": 1.1}  # other kwargs for gpt/request calling
        self.call_kwargs_gpt = {"max_completion_tokens": 8192, "temperature": 0.1} #"temperature": 0.0, "top_p": 0.95, 
        # --
    ...
    def _call_with_messages(self, messages, **kwargs):
        ...
        if _call_target_type == "gpt":
            _call_kwargs = self.call_kwargs_gpt.copy()
        else:
        ...
        elif _call_target_type == "gemini":
            ret = self._call_gemini_chat(messages, **_call_kwargs)
    ...
    def _call_openai_chat(self, messages, **kwargs):
        ...
        # kwargs = {"max_completion_tokens": 8192, "temperature": 0.1} 
    ...
    def _call_gemini_chat(self, messages, **kwargs):
        _gemini_kwargs = {"model": self.call_target.split(":", 1)[1]}
        _gemini_kwargs.update(kwargs)
        while True:
            try:
                ret = GeminiHelper().call_chat(messages, stat=self.call_stat, **_gemini_kwargs)
                return ret
            # except Exception as e:  # simply catch everything!
            except Exception as e:  # simply catch everything!
                rprint(f"Get error when calling gemini: {e}", style="white on red")
        return
        #ret = GeminiHelper().call_chat(messages, stat=self.call_stat, **_gemini_kwargs)
...
# PYTHONPATH=???/cognitive_kernel_v3/backend python -m ckv3.agents.model
