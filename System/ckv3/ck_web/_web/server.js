...
const processNextInQueue = async () => {
  ...
  if (waitingQueue.length > 0 && availableBrowserslot) {
    ...
    try {
      ...
      if (nextRequest && nextRequest.res) {
        nextRequest.res.status(500).send({ error: 'Failed to allocate browser.' });
    } else {
        console.error('Request object is missing res property');
    }
    }
  ...
  try {
    ...
    if (!browserEntry.browser) {
      ...
      if (
        ...
        const wsEndpoint = `ws://localhost:${PROXY_PORT}/chromium/playwright?token=${BROWSERLESS_TOKEN}&timeout=300000`;
        new_browser = await chromium.connect(wsEndpoint, { timeout: 300000 });
    ...
    page.on('download', (download) => {
      ...
      downloadPromises.push(
        download.saveAs(filePath)
          ...
          .catch(error => {
            // Do NOT re-throw: an unhandled rejection will crash the process.
            console.error('Download save error (non-fatal, skipped):', error.message || error);
            return null;
        })
      );
    });
